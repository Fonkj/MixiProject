...
class SpriteManager:
    def __init__(self):
        # Using the new helper method to clean images on the fly
        ...
        # Clean the flip-flop background too
        self.flipflop_image = self.load_and_clean_image("image3.png", (50, 50))
    ...
    def load_and_clean_image(self, path, size=None):
        """Helper to remove background and return a Pygame surface"""
        try:
            ...
            # Convert the byte data back into a format Pygame understands
            ...
            if size:
                return pygame.transform.scale(py_image, size)
            return py_image
        except Exception as e:
            print(f"Error cleaning {path}: {e}")
            # Return a placeholder if it fails
            surf = pygame.Surface(size if size else (50, 50))
            surf.fill((255, 0, 255))
            return surf

    def load_character_animations(self, name, image_filename):
        # Call our cleaning helper instead of standard pygame.load
        full_img = self.load_and_clean_image(image_filename)
        
        scale_height = 160 
        aspect_ratio = full_img.get_width() / full_img.get_height()
        full_img = pygame.transform.scale(full_img, (int(scale_height * aspect_ratio), scale_height))
